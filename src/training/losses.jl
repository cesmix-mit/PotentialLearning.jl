...
"""
loss(es_pred, es, w_e, fs_pred, fs, w_f) = w_e * loss(es_pred, es) + w_f * loss(fs_pred, fs)
...
"""
global_loss(loader_e, loader_f, w_e, w_f, ps, re) = mean([
    loss(
        potential_energy.(bs_e, [ps], [re]),
        es,
        w_e,
        force.(bs_f, dbs_f, [ps], [re]),
        fs,
        w_f,
...
"""
global_energy_loss(loader_e, w_e, ps, re) =
    mean([w_e * loss(potential_energy.(bs_e, [ps], [re]), es) for (bs_e, es) in loader_e])
...
"""
global_force_loss(loader_f, w_f, ps, re) = mean([
    w_f * loss(force.(bs_f, dbs_f, [ps], [re]), fs) for (bs_f, dbs_f, fs) in loader_f
])
