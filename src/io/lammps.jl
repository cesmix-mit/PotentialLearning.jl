...
function load_data(file, lammps::LAMMPS)
    ...
    box = [[1.0, 0.0, 0.0] * 1uu, [0.0, 1.0, 0.0] * 1uu, [0.0, 0.0, 1.0] * 1uu]
    positions = SVector{3}[]
    ...
    system = open(file, "r") do io
        while !eof(io)
            ...
            if ("atom" in line) && ("types" in line)
                ...
                if num_atom_types != length(lammps.elements)
                    error(
                        "num_atom_types in LAMMPS file does not match number of elements in LAMMPS struct!",
                    )
                end
            end
            if "xlo" in line
        ...
        for i = 1:num_atoms
            if isempty(velocities)
                ...
                atoms[i] = Atom(
                    lammps.elements[elements[i]],
                    positions[i] * 1uu,
                    velocities[i] * 1uu / lammps.time_units,
                )
            end
        end
