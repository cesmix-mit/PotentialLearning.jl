...
models = Dict()
for s in species
    nn = Chain( Dense(n_desc,16,σ; init = Flux.glorot_normal),
                Dense(16,16,σ; init = Flux.glorot_normal),
                Dense(16,1; init = Flux.glorot_normal))
    models[s] = nn
end
npod = NNIAP(models, pod)


# Set initial parameters from previous training
