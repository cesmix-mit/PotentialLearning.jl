...
scatter!(ax1, e_true_train[dpp_inds2], errors_train[dpp_inds2], color = :darkred, label = "DPP Samples", markersize = 5.0)
...
# Import configurations 
ds_ace, thermo_ace = load_data("examples/Sodium/data/liquify_sodium_ace.yaml", YAML(u"eV", u"Å"));
ds_ace, thermo_ace = ds_ace[220:end], thermo_ace[220:end]
systems_ace = get_system.(ds_ace);
local_descriptors_ace = LocalDescriptors.(JLD.load("examples/Sodium/data/sodium_ace_md_descriptors.jld", "local_descriptors"))
...
ax2 = Axis(fig[1,1], yaxisposition = :right, ylabel = "Probability of Retraining", ylabelcolor = :red)
